...
const factory: CustomToolFactory = () => ({
	...
	description:
		...
		"Run a Taleb-style pre-mortem: assume the decision has utterly failed and reverse-engineer failure paths. " +
	...
	parameters: Type.Object({
		...
		time_horizon: Type.Optional(
			Type.String({
				description: "时间锚。默认 3 年。Time horizon, defaults to 3 years.",
				examples: ["1 year", "3 years", "5 years", "1 年", "3 年", "5 年"],
			}),
		),
	}),
	async execute(_toolCallId, params) {
		const horizon = params.time_horizon ?? "3 年 / 3 years";
		const lines: string[] = [];
		lines.push(`# 死亡预演指令 / Pre-Mortem Instruction\n`);
